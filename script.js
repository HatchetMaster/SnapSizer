/* script.js - Tiny Image Resizer (robust debug-friendly) */
...
let files = []; // {file, img, name, origWidth, origHeight, outputBlob, outputName}
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            reader.onload = (ev) => {
                ...
                const img = new Image();
                // Ensure no CSS affects intrinsic size: we don't add it to DOM before reading natural sizes.
                img.onload = () => {
                    // Use naturalWidth / naturalHeight for accurate intrinsic pixel size
...
document.addEventListener('click', (e) => {
    // if click is inside a thumb, don't close (handled separately)
...
function renderThumbs() {
    ...
    files.forEach((it, idx) => {
        ...
        menuButton.innerHTML = '⋯'; // simple icon (you can swap for SVG)
        ...
        function openMenu() {
            ...
            // optional: flip up if near bottom of viewport
        ...
        div.addEventListener('click', (ev) => {
            // ignore clicks on the menu/button (handled)
        ...
        // place menu into actions so it floats relative to thumb
...
function calcTarget(origW, origH, cfg) {
    ...
    // fit: clamp inside width/height bounds (if either provided). If allowUpscale==true, do not clamp by 1.
    if (cfg.mode === 'fit') {
        ...
        if (cfg.keepAspect) {
            ...
            const ratio = cfg.allowUpscale ? ratioBase : Math.min(ratioBase, 1); // <-- key change
...
async function resizeImage(img, cfg, originalName, origType) {
    ...
    if (typeof pica !== 'undefined') {
        try {
            ...
            // quality options: lower to faster; unsharp helps downscales
    ...
    try {
        ...
        const blob = await new Promise(resolve => {
            try {
                ...
                // toBlob may throw on weird browsers — fallback to toDataURL
                try {
